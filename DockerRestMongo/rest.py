...
import sys, os, re, shutil, json, urllib, urllib2, BaseHTTPServer,pymongo,request
...
def get_records(handler):   
    ...
    for s in collection.find():
        output.append({'Title' : s['Title'], 'email' : s['email'], 'Password': s['Password']})
    return output

def get_record(handler):
    key = urllib.unquote(handler.path[8:])
    s = collection.find_one({'email':key})
    if s:
        output = {'Title' : s['title'], 'email' : s['email'], 'Password': s['Password']}
    else:
        output = "No such name"
    return output
...
def set_record(handler):
    ...
    account_id = collection.insert({'Title' : payload['Title'], 'email' : payload['email'], 'Password': payload['Password']})
    new_account = collection.find_one({'_id': account_id})
    output = {'Title' : new_account['Title'], 'email' : new_account['email'], 'Password': new_account['Password']}
    return output

def delete_record(handler):
    key = urllib.unquote(handler.path[8:])
    collection.remove({"email": key})
